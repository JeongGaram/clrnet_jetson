import os, time, warnings, threading, math
...
def write_points(lanes_xys, result_img_name):
    ...
    for xys in lanes_xys:
        if len(xys) >= 2 :
            ...
            temp = ' '.join(str(x) for x in sum(xys,()))
            temp2 = ' '.join(str(x) for x in sum(extract_start_end_points,()))
            lanes_txt.append(temp)
            lanes_txt2.append(temp2)
        
    with open(result_img_name.replace('.jpg','.lines.txt'), 'w', encoding='UTF-8') as f:
        for points in lanes_txt:
            f.write(points + '\n')
    with open(result_img_name.replace('.jpg','.lines2.txt'), 'w', encoding='UTF-8') as f:
        for points in lanes_txt2:
            f.write(points + '\n')
...
# plot direction
def draw_direction(img, car_center_points, car_direction, img_car_width, color, theta):
    if car_direction is not None:
        theta = np.radians(-1*car_direction)
    ...
    # arrow coodination
    arrow_center_x = round(math.cos(theta+math.pi/2)*200)
    arrow_center_y = round(math.sin(theta+math.pi/2)*200)
    left = [-30, 200-30]
    right = [30, 200-30]
    rot_left = np.dot(Rot_mat, np.array(left))
    rot_right = np.dot(Rot_mat, np.array(right))
    
    arrow_left_x, arrow_left_y = car_center_points[0]+rot_left[0], car_center_points[1]-rot_left[1]
    arrow_right_x, arrow_right_y = car_center_points[0]+rot_right[0], car_center_points[1]-rot_right[1]

    # 왼쪽
    img = cv2.line(img, 
                   (int(car_center_points[0]+arrow_center_x-img_car_width/2), int(car_center_points[1]-arrow_center_y)), 
                   (int(round(arrow_left_x-img_car_width/2)), int(round(arrow_left_y))), color=color, thickness=5)
    img = cv2.line(img, 
                   (int(car_center_points[0]+arrow_center_x-img_car_width/2), int(car_center_points[1]-arrow_center_y)), 
                   (int(round(arrow_right_x-img_car_width/2)), int(round(arrow_right_y))), color=color, thickness=5)
    img = cv2.line(img, 
                   (int(car_center_points[0]-img_car_width/2), int(car_center_points[1])), 
                   (int(car_center_points[0]+arrow_center_x-img_car_width/2), int(car_center_points[1]-arrow_center_y)), color=color, thickness=5)
...
def run(image, img_count, original_img_dir, result_img_dir, theta):
    ...
    if not scaled_lines:
        ...
        # plot direction
        car_direction = None
        image, theta = draw_direction(image, car_center_points, car_direction, img_car_width, [255,255,255], theta)
        ...
        # plot direction
        car_direction = ref_line_angle-extracted_line_angle
        image, theta = draw_direction(image, car_center_points, car_direction, img_car_width, [255,255,255], theta)
...
if __name__ == "__main__":
    ...
    if input_type[0] == 'camera':
        ...
        file_name = 'IMG_2459.mp4'
    
    ori_video_dir = f'./{file_name}' 
    # ori_video_dir = './data/video/video.mp4' 
    result_img_dir = f'./results/real_time/{file_name+"_trt"}/test' # image save directory
    original_img_dir = f'./results/real_time/{file_name+"_trt"}/original_image'
    real_time_img_dir = f'./results/real_time/{file_name+"_trt"}/real_time_image'
    ...
    #clr = CLRNetDemo('./agri_r18.engine.jetson')
    #clr = CLRNetDemo('./49_230206.engine')
    clr = CLRNetDemo('./49_230206.engine.jetson')
    ...
    start = time.time()
    #isnet = CLRNetDemo('./agri_r18.engine')
    ...
    #     os.makedirs('./results/'+os.path.split(img)[0], exist_ok=True)
    #     image = cv2.imread(img)
    #     output, lanes_xys = isnet.forward(image)
    #     result_img = os.path.join('results',img)
    #     write_points(lanes_xys, result_img)
    #     cv2.imwrite(result_img, output)
    #     #end = time.time()
    #     #elapsed_time = end-start
    #     #print(f"fps : {1/elapsed_time}, {elapsed_time:.4f} sec")
    ...
    while(cap.isOpened()):
        ...
        if True:
            ...
            # print(img_count)
            theta = run(image, img_count, original_img_dir, result_img_dir, theta)
