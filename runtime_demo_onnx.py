...
if __name__ == "__main__":
    start = time.time()
    clr = CLRNetDemo('./agri_r18.onnx')
    ori_img_dir = 'test_img'
    result_img_dir = 'results/onnx'
    for dir in os.listdir(ori_img_dir):
        if 'seq' in dir:
            ori_seq_dir = os.path.join(ori_img_dir, dir)
            result_seq_dir = os.path.join(result_img_dir, dir)
            os.makedirs(result_seq_dir, exist_ok=True)
            print(f'==== {dir} ====')
            for img in sorted(os.listdir(ori_seq_dir)):
                if '.jpg' in img:
                    #start = time.time()
                    test_img = os.path.join(ori_seq_dir, img)
                    image = cv2.imread(test_img)
                    output, lanes_xys = clr.forward(image)
                    result_img_name = os.path.join(result_seq_dir, img)

                    write_points(lanes_xys, result_img_name)
                    cv2.imwrite(result_img_name, output)
                    #end = time.time()
